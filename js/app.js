...
function loadProducts(json, sortType) {
  ...
  productChilds = Array.from(productElement.querySelectorAll(".product"));
...
function loadCombo(combo, data) {
  data.map((t) =>
    combo.insertAdjacentHTML("beforeend", `<option>${t}</option>`)
  );
}
...
filterSort.addEventListener("change", (e) => {
  loadProducts(productsAll, filterSort.value);
  loadFilters();
});
